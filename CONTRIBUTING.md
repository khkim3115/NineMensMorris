...
- 이름: `<type>/<이슈번호>-<짧은-설명>`
  - 예) `feat/25-patch-notes`, `fix/31-mill-removal`, `chore/40-deps-bump`, `docs/42-readme`
...
- 형식: `<type>(<scope>): <한국어 설명> (#이슈)`
  - 예) `feat(engine): 어려움 AI 전치표 추가 (#25)`
- `scope` 예: `ui`, `core`, `engine`, `solo`, `mp`, `desktop`, `web`, `deps` …
...
- `src/core` 를 손댔다면 `src/engine` 강도 회귀 테스트까지 반드시 확인한다.
- `src/core`·`src/engine` 변경 후에는 `npm run build:tray-engine` 으로 트레이 번들을 다시 만들어 커밋한다.
